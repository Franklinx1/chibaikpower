/* ============================================================
   CHIBAIK POWER — script.js
   Single clean file. No conflicts. No duplicate listeners.
============================================================ */
...
function initQuiz() {
  if (localStorage.getItem('chibaik-quiz-done')) return;
  setTimeout(() => {
...
function quizNext(step, answer) {
  quizData['step' + step] = answer;
  document.getElementById('qs' + step).classList.remove('on');
  document.getElementById('qd' + step).classList.add('on');
  const next = step + 1;
  const nextEl = document.getElementById('qs' + next);
  const nextDot = document.getElementById('qd' + next);
  if (nextEl) { nextEl.classList.add('on'); if (nextDot) nextDot.classList.add('on'); }
}

function quizFinish(goal) {
  quizData.goal = goal;
  localStorage.setItem('chibaik-quiz-done', '1');
  localStorage.setItem('chibaik-quiz-data', JSON.stringify(quizData));
  closeQuiz();
  setTimeout(() => {
    if (goal === 'independence' || quizData.step1 === 'house') {
      showToast('⚡ Based on your answers, check out Let There Be Light');
    } else if (goal === 'backup' || quizData.step1 === 'apartment') {
      showToast('⚡ A Solar Power Station might be perfect for you');
    } else {
      showToast('⚡ Use the load calculator to find your perfect match');
    }
  }, 500);
}
...
function calcFuel() {
  ...
  else if (mo < 40000) ins = `You are spending <strong>${fmtN(mo)} every month</strong> on fuel. That is <strong>${fmtN(yr)} a year.</strong> A mid-range solar system could pay itself off in roughly ${Math.round(500000 / mo)} months.`;
  else if (mo < 100000) ins = `<strong>${fmtN(mo)} every single month</strong> is going to fuel. That is <strong>${fmtN(five)} over 5 years.</strong> A full home solar system could eliminate that cost entirely.`;
...
const appData = [
  ...
  { id:'strip',     cat:'light',   img:'🌈', name:'LED Strip Light',         v:[{l:'Small Set 12W',w:12},{l:'Medium Set 20W',w:20},{l:'Large Set 36W',w:36}] },
  ...
  { id:'ac',        cat:'cooling', img:'❄️', name:'Air Conditioner',         v:[{l:'0.75HP Window',w:600},{l:'1HP Split',w:750},{l:'1.5HP Split',w:1200},{l:'2HP Split',w:1800},{l:'2.5HP Inverter',w:2000}] },
  ...
  { id:'micro',     cat:'kitchen', img:'📦', name:'Microwave Oven',          v:[{l:'700W Rated',w:1000},{l:'900W Rated',w:1200},{l:'1100W Rated',w:1400}] },
  ...
  { id:'phone',     cat:'office',  img:'📱', name:'Phone Charger',           v:[{l:'Single 15W',w:15},{l:'Fast Charge 30W',w:30},{l:'Multiple Phones 60W',w:60}] },
...
const pkgs = [
  { n:'Starter Package',    inv:'1kW Inverter',        bat_li:'100Ah / 12V LiFePO4 Battery',       bWh_li:1200, bat_tu:'200Ah / 12V Tubular Battery',        bWh_tu:1000, ctrl_li:'20A MPPT Charge Controller', ctrl_tu:'20A MPPT Charge Controller', panelTotalW:500,  max:1000 },
  { n:'Standard Package',   inv:'2kW Inverter',        bat_li:'200Ah / 12V LiFePO4 Battery',       bWh_li:2400, bat_tu:'2x200Ah / 12V Tubular Batteries',    bWh_tu:2000, ctrl_li:'40A MPPT Charge Controller', ctrl_tu:'40A MPPT Charge Controller', panelTotalW:1000, max:2000 },
  { n:'Premium Package',    inv:'3kW Hybrid Inverter', bat_li:'200Ah / 24V LiFePO4 Battery',       bWh_li:4800, bat_tu:'400Ah / 24V Tubular Battery',        bWh_tu:4000, ctrl_li:'Built-in (Hybrid Inverter)', ctrl_tu:'Built-in (Hybrid Inverter)', panelTotalW:1800, max:3000 },
  { n:'Heavy Duty Package', inv:'5kW Hybrid Inverter', bat_li:'400Ah / 24V LiFePO4 Battery',       bWh_li:9600, bat_tu:'800Ah / 24V Tubular Battery',        bWh_tu:8000, ctrl_li:'Built-in (Hybrid Inverter)', ctrl_tu:'Built-in (Hybrid Inverter)', panelTotalW:3000, max:5000 },
];
...
const sel = {};
let totalLoad = 0, activeCat = 'all', waItemText = '', waLoad = 0;
let solarPanelW = 300, solarBattType = 'lithium', solarBattSizeKwh = 5, solarBattCount = 1, solarPanelCount = 2;
const PEAK_SUN_HOURS = 5;

// ═══ SVG ICONS ═══
const svgMap = {
  'bulb-led': `<svg viewBox="0 0 80 80" fill="none"><path d="M40 10C25 10 15 22 15 34c0 10 7 17 13 22v6c0 2 2 4 5 4h14c3 0 5-2 5-4v-6c6-5 13-12 13-22C65 22 55 10 40 10z" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.08)"/><line x1="30" y1="66" x2="30" y2="70" stroke="#FFD000" stroke-width="2"/><line x1="40" y1="66" x2="40" y2="70" stroke="#FFD000" stroke-width="2"/><line x1="50" y1="66" x2="50" y2="70" stroke="#FFD000" stroke-width="2"/><path d="M31 70v4c0 2 3 3 9 3s9-1 9-3v-4" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/></svg>`,
  'bulb-cfl': `<svg viewBox="0 0 80 80" fill="none"><path d="M30 65V42q0-14-7-20-5-5-5-8 0-6 8-6 8 0 10 12h14q2-12 10-12 8 0 8 6 0 3-5 8-7 6-7 20v23" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><line x1="32" y1="65" x2="32" y2="69" stroke="#FFD000" stroke-width="2"/><line x1="40" y1="65" x2="40" y2="69" stroke="#FFD000" stroke-width="2"/><line x1="48" y1="65" x2="48" y2="69" stroke="#FFD000" stroke-width="2"/><path d="M33 69v4c0 2 3 3 7 3s7-1 7-3v-4" stroke="#FFD000" stroke-width="2"/></svg>`,
  ...
  'fan-s':    `<svg viewBox="0 0 80 80" fill="none"><circle cx="40" cy="26" r="20" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.06)"/><circle cx="40" cy="26" r="5" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.2)"/><line x1="40" y1="46" x2="40" y2="64" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/><path d="M24 64q0 8 16 10q16-2 16-10" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/></svg>`,
  'fan-t':    `<svg viewBox="0 0 80 80" fill="none"><circle cx="40" cy="30" r="17" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.06)"/><circle cx="40" cy="30" r="4" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.2)"/><line x1="40" y1="47" x2="40" y2="58" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/><rect x="22" y="58" width="36" height="10" rx="4" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.08)"/></svg>`,
  'fan-e':    `<svg viewBox="0 0 80 80" fill="none"><rect x="10" y="10" width="60" height="60" rx="8" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.06)"/><circle cx="40" cy="40" r="22" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.04)"/><circle cx="40" cy="40" r="6" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.15)"/></svg>`,
  'ac':       `<svg viewBox="0 0 80 80" fill="none"><rect x="4" y="22" width="72" height="36" rx="6" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><line x1="12" y1="36" x2="62" y2="36" stroke="#FFD000" stroke-width="1.5"/><line x1="12" y1="42" x2="62" y2="42" stroke="#FFD000" stroke-width="1.5"/><line x1="12" y1="48" x2="62" y2="48" stroke="#FFD000" stroke-width="1.5"/><circle cx="70" cy="32" r="2.5" fill="#FFD000"/><circle cx="70" cy="40" r="2.5" fill="rgba(255,208,0,.3)"/></svg>`,
  'fridge-m': `<svg viewBox="0 0 80 80" fill="none"><rect x="18" y="10" width="44" height="62" rx="5" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><line x1="18" y1="30" x2="62" y2="30" stroke="#FFD000" stroke-width="2"/><path d="M52 16v11" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/><path d="M52 34v30" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/></svg>`,
  'fridge':   `<svg viewBox="0 0 80 80" fill="none"><rect x="14" y="6" width="52" height="68" rx="5" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><line x1="14" y1="28" x2="66" y2="28" stroke="#FFD000" stroke-width="2"/><path d="M54 12v12" stroke="#FFD000" stroke-width="3.5" stroke-linecap="round"/><path d="M54 32v36" stroke="#FFD000" stroke-width="3.5" stroke-linecap="round"/></svg>`,
  'freezer':  `<svg viewBox="0 0 80 80" fill="none"><rect x="6" y="28" width="68" height="42" rx="6" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><path d="M16 28l2-14h44l2 14" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.07)"/><circle cx="40" cy="52" r="10" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/></svg>`,
  'tv':       `<svg viewBox="0 0 80 80" fill="none"><rect x="6" y="14" width="68" height="42" rx="4" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><rect x="10" y="18" width="60" height="34" rx="2" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.04)"/><path d="M30 56l-4 14h28l-4-14" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.07)"/><line x1="18" y1="70" x2="62" y2="70" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/><circle cx="68" cy="34" r="2" fill="#FFD000"/></svg>`,
  'decoder':  `<svg viewBox="0 0 80 80" fill="none"><rect x="8" y="26" width="64" height="28" rx="4" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><rect x="14" y="31" width="28" height="18" rx="2" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.08)"/><circle cx="54" cy="40" r="6" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><circle cx="54" cy="40" r="2" fill="#FFD000"/><circle cx="66" cy="34" r="2" fill="#FFD000"/><circle cx="66" cy="40" r="2" fill="rgba(255,208,0,.3)"/><circle cx="66" cy="46" r="2" fill="rgba(255,208,0,.3)"/></svg>`,
  'sound':    `<svg viewBox="0 0 80 80" fill="none"><rect x="14" y="8" width="52" height="64" rx="6" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><circle cx="40" cy="36" r="18" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.05)"/><circle cx="40" cy="36" r="10" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><circle cx="40" cy="36" r="4" fill="rgba(255,208,0,.35)"/><circle cx="40" cy="62" r="5" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.1)"/></svg>`,
  'console':  `<svg viewBox="0 0 80 80" fill="none"><path d="M8 46q0 10 8 14l14 4q6 2 10-4q4 6 10 4l14-4q8-4 8-14L68 28q-2-8-10-8H22Q14 20 12 28z" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><line x1="22" y1="38" x2="22" y2="50" stroke="#FFD000" stroke-width="2" stroke-linecap="round"/><line x1="16" y1="44" x2="28" y2="44" stroke="#FFD000" stroke-width="2" stroke-linecap="round"/><circle cx="52" cy="38" r="3" stroke="#FFD000" stroke-width="1.5"/><circle cx="60" cy="42" r="3" stroke="#FFD000" stroke-width="1.5"/></svg>`,
  'micro':    `<svg viewBox="0 0 80 80" fill="none"><rect x="6" y="16" width="68" height="48" rx="6" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><rect x="10" y="20" width="48" height="40" rx="4" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.04)"/><circle cx="34" cy="40" r="14" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.08)"/><circle cx="34" cy="40" r="7" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.12)"/><circle cx="66" cy="40" r="3" fill="#FFD000"/></svg>`,
  'kettle':   `<svg viewBox="0 0 80 80" fill="none"><path d="M22 22q-4 18-2 36h36q2-18-2-36z" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><path d="M26 22h28q2-5 0-8H26q-2 3 0 8z" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><path d="M56 30q12-4 14-10" stroke="#FFD000" stroke-width="3" stroke-linecap="round" fill="none"/><ellipse cx="40" cy="60" rx="22" ry="5" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/></svg>`,
  'blender':  `<svg viewBox="0 0 80 80" fill="none"><path d="M28 14l-4 38h32l-4-38z" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><path d="M32 14h16q2-4 0-6H32q-2 2 0 6z" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><rect x="20" y="52" width="40" height="12" rx="4" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.1)"/><rect x="28" y="64" width="24" height="8" rx="3" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.07)"/></svg>`,
  'rice':     `<svg viewBox="0 0 80 80" fill="none"><ellipse cx="40" cy="52" rx="28" ry="14" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><path d="M12 52V44q0-24 28-24q28 0 28 24v8" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><ellipse cx="40" cy="22" rx="16" ry="4" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.12)"/></svg>`,
  'iron':     `<svg viewBox="0 0 80 80" fill="none"><path d="M10 56V46q0-10 12-14h40q12 0 10 12l-8 12z" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><path d="M26 32V26q0-4 6-4h16q6 0 6 4v6" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.07)"/><circle cx="24" cy="50" r="2" fill="rgba(255,208,0,.4)"/><circle cx="32" cy="50" r="2" fill="rgba(255,208,0,.4)"/><circle cx="40" cy="50" r="2" fill="rgba(255,208,0,.4)"/><circle cx="48" cy="50" r="2" fill="rgba(255,208,0,.4)"/></svg>`,
  'toaster':  `<svg viewBox="0 0 80 80" fill="none"><rect x="10" y="34" width="60" height="32" rx="6" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><rect x="22" y="22" width="14" height="20" rx="3" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><rect x="44" y="22" width="14" height="20" rx="3" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><circle cx="62" cy="46" r="5" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.08)"/><circle cx="62" cy="46" r="2" fill="#FFD000"/></svg>`,
  'cooker':   `<svg viewBox="0 0 80 80" fill="none"><rect x="8" y="46" width="64" height="22" rx="5" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><circle cx="28" cy="32" r="16" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><circle cx="28" cy="32" r="8" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.15)"/><circle cx="60" cy="32" r="10" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.07)"/><circle cx="60" cy="32" r="5" fill="rgba(255,208,0,.15)"/></svg>`,
  'disp':     `<svg viewBox="0 0 80 80" fill="none"><ellipse cx="40" cy="16" rx="16" ry="8" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.12)"/><rect x="22" y="26" width="36" height="46" rx="4" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><circle cx="40" cy="42" r="10" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.08)"/><rect x="28" y="54" width="10" height="7" rx="2" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.1)"/><rect x="42" y="54" width="10" height="7" rx="2" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.1)"/></svg>`,
  'laptop':   `<svg viewBox="0 0 80 80" fill="none"><path d="M12 12h56l-3 38H15z" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><path d="M8 50h64l4 12H4z" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.08)"/><rect x="28" y="54" width="24" height="4" rx="2" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.1)"/></svg>`,
  'desktop':  `<svg viewBox="0 0 80 80" fill="none"><rect x="8" y="8" width="64" height="44" rx="4" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><rect x="12" y="12" width="56" height="36" rx="2" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.04)"/><path d="M28 52l-4 12" stroke="#FFD000" stroke-width="2.5" stroke-linecap="round"/><path d="M52 52l4 12" stroke="#FFD000" stroke-width="2.5" stroke-linecap="round"/><line x1="18" y1="64" x2="62" y2="64" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/></svg>`,
  'printer':  `<svg viewBox="0 0 80 80" fill="none"><rect x="8" y="28" width="64" height="30" rx="5" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><path d="M18 28V14h44v14" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.07)"/><path d="M22 48v14h36V48" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.07)"/><line x1="28" y1="52" x2="52" y2="52" stroke="#FFD000" stroke-width="1.5"/><circle cx="60" cy="38" r="3" fill="#FFD000"/></svg>`,
  'router':   `<svg viewBox="0 0 80 80" fill="none"><rect x="10" y="44" width="60" height="22" rx="5" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><line x1="22" y1="44" x2="18" y2="18" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/><line x1="40" y1="44" x2="40" y2="14" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/><line x1="58" y1="44" x2="62" y2="18" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/><circle cx="18" cy="16" r="3.5" fill="#FFD000"/><circle cx="40" cy="12" r="3.5" fill="#FFD000"/><circle cx="62" cy="16" r="3.5" fill="#FFD000"/></svg>`,
  'phone':    `<svg viewBox="0 0 80 80" fill="none"><rect x="22" y="6" width="36" height="56" rx="6" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><rect x="26" y="10" width="28" height="44" rx="3" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.04)"/><circle cx="40" cy="66" r="4" stroke="#FFD000" stroke-width="2"/><path d="M40 62v12q0 4-4 4" stroke="#FFD000" stroke-width="2" stroke-linecap="round" fill="none"/><rect x="28" y="74" width="16" height="6" rx="3" stroke="#FFD000" stroke-width="1.5" fill="rgba(255,208,0,.07)"/></svg>`,
  'cctv':     `<svg viewBox="0 0 80 80" fill="none"><ellipse cx="34" cy="32" rx="22" ry="16" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><circle cx="30" cy="32" r="10" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><circle cx="30" cy="32" r="4" fill="rgba(255,208,0,.3)"/><path d="M50 24l16-8v32l-16-8" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.07)"/><path d="M34 48v14" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/><line x1="20" y1="62" x2="48" y2="62" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/></svg>`,
  'pump':     `<svg viewBox="0 0 80 80" fill="none"><rect x="14" y="28" width="40" height="28" rx="6" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><circle cx="34" cy="42" r="10" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><circle cx="34" cy="42" r="4" fill="rgba(255,208,0,.25)"/><path d="M54 38h12v-8h8" stroke="#FFD000" stroke-width="3" stroke-linecap="round" fill="none"/><path d="M54 46h12v8h8" stroke="#FFD000" stroke-width="3" stroke-linecap="round" fill="none"/></svg>`,
  'washer':   `<svg viewBox="0 0 80 80" fill="none"><rect x="8" y="8" width="64" height="68" rx="6" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><rect x="8" y="8" width="64" height="18" rx="6" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><circle cx="22" cy="17" r="4" stroke="#FFD000" stroke-width="2"/><circle cx="35" cy="17" r="4" fill="#FFD000"/><circle cx="40" cy="50" r="22" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.04)"/><circle cx="40" cy="50" r="10" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/></svg>`,
  'hairdryer':`<svg viewBox="0 0 80 80" fill="none"><ellipse cx="38" cy="30" rx="26" ry="18" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><circle cx="26" cy="30" r="10" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><path d="M38 48q-4 4-8 12q-2 6 2 8q6 0 8-6l2-14" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><line x1="56" y1="24" x2="64" y2="20" stroke="#FFD000" stroke-width="2" stroke-linecap="round"/><line x1="58" y1="30" x2="68" y2="30" stroke="#FFD000" stroke-width="2" stroke-linecap="round"/><line x1="56" y1="36" x2="64" y2="40" stroke="#FFD000" stroke-width="2" stroke-linecap="round"/></svg>`,
  'heater':   `<svg viewBox="0 0 80 80" fill="none"><rect x="24" y="10" width="32" height="56" rx="8" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><circle cx="40" cy="42" r="12" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.08)"/><circle cx="40" cy="42" r="6" fill="rgba(255,208,0,.2)"/><line x1="10" y1="60" x2="22" y2="60" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/><line x1="58" y1="60" x2="70" y2="60" stroke="#FFD000" stroke-width="3" stroke-linecap="round"/></svg>`,
  'fryer':    `<svg viewBox="0 0 80 80" fill="none"><path d="M14 42q0-22 26-22q26 0 26 22v14q0 14-26 14q-26 0-26-14z" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><ellipse cx="40" cy="42" rx="22" ry="10" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><circle cx="60" cy="54" r="5" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><circle cx="60" cy="54" r="2" fill="#FFD000"/></svg>`,
  'grinder':  `<svg viewBox="0 0 80 80" fill="none"><ellipse cx="40" cy="20" rx="22" ry="8" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.1)"/><path d="M18 20v32q0 14 22 14q22 0 22-14V20" stroke="#FFD000" stroke-width="2.5" fill="rgba(255,208,0,.07)"/><ellipse cx="40" cy="52" rx="22" ry="8" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/><rect x="36" y="8" width="8" height="12" rx="2" stroke="#FFD000" stroke-width="2" fill="rgba(255,208,0,.1)"/></svg>`,
};
...
function showRes() {
  ...
  showToast('⚡ ' + totalLoad.toLocaleString() + 'W load — here are your options');
}
...
function chQ(id, d) {
  ...
  const qn = document.getElementById('aqn-' + id);
...
function fmtD(h) {
  if (!isFinite(h) || h <= 0) return 'less than 1 min';
...
function buildGenRecs() {
  ...
  if (!matches.length) {
    grid.innerHTML = `<div class="no-match"><p>Your load of <strong>${totalLoad.toLocaleString()}W</strong> is too high for standard power stations. Switch to the <strong>Let There Be Light</strong> tab for a full solar system.</p></div>`;
    return;
  }
  const ref = Math.max(...matches.map(s => s.cap / (totalLoad * 0.25 || 1)));
  grid.innerHTML = matches.map(s => {
    const yH = s.cap / (totalLoad || 1), mH = s.cap / s.max, lH = s.cap / (totalLoad * 0.3 || 1);
    return `<div class="rcard" onclick="openWa('${s.r}','${s.r}',${totalLoad})">
      ...
      <div class="dur-row"><div class="dur-lbl">Full Power (${s.max.toLocaleString()}W) <span>${fmtD(mH)}</span></div><div class="dur-track"><div class="dur-fill" style="width:${bW(mH,ref)}%;background:linear-gradient(90deg,#FF4444,#FF8C00)"></div></div></div>
      <div class="dur-row" style="margin-bottom:12px"><div class="dur-lbl">Light Use 30% <span>${fmtD(lH)}</span></div><div class="dur-track"><div class="dur-fill" style="width:${bW(lH,ref)}%;background:linear-gradient(90deg,#00C853,#00E676)"></div></div></div>
...
function buildSolRecs() {
  ...
  const userBatStr = `${solarBattCount} x ${selectedSize.label} ${solarBattType === 'lithium' ? 'LiFePO4' : 'Tubular'} Batter${solarBattCount > 1 ? 'ies' : 'y'}`;
  grid.innerHTML = matches.map(p => {
    const ctrl = solarBattType === 'lithium' ? p.ctrl_li : p.ctrl_tu;
    const yH = userBattWh / (totalLoad || 1), fH = userBattWh / p.max, lH = userBattWh / (totalLoad * 0.3 || 1);
    const ref = userBattWh / (totalLoad * 0.25 || 1);
...
function changeBattCount(delta) {
  ...
  const el = document.getElementById('battCount');
  if (el) el.textContent = solarBattCount;
  updateCounterInsights();
  buildSolRecs();
}
...
function changePanelCount(delta) {
  ...
  if (el) el.textContent = solarPanelCount;
  updateCounterInsights();
  buildSolRecs();
}
...
function updateCounterInsights() {
  ...
  if (battEl) battEl.textContent = totalKwh + 'kWh';
  ...
  if (battInsightEl && totalLoad > 0) {
    ...
    const sizeNote = (solarBattType === 'lithium' ? lithiumSizes : tubularSizes).find(s => s.kWh === solarBattSizeKwh)?.note || '';
    battInsightEl.innerHTML = `With <strong>${solarBattCount} x ${solarBattType === 'lithium' ? solarBattSizeKwh + 'kWh lithium' : (tubularSizes.find(s=>s.kWh===solarBattSizeKwh)?.label||'')}</strong> your backup at <strong>${totalLoad.toLocaleString()}W</strong> is approximately <strong>${fmtD(hours)}</strong>. ${sizeNote}`;
    battInsightEl.classList.add('show');
  } else if (battInsightEl) battInsightEl.classList.remove('show');
  ...
  if (panelInsightEl) {
    ...
    panelInsightEl.innerHTML = `<strong>${solarPanelCount} x ${solarPanelW}W panels</strong> generate <strong>${panelDailyKwh.toFixed(1)}kWh per day</strong> in Nigeria (5 peak sun hours average). They can fully charge your <strong>${totalBattKwh.toFixed(1)}kWh of storage</strong> in approximately <strong>${fmtD(chargeHours)}</strong> of direct sun, replenishing <strong>${coveragePercent}%</strong> of your total battery each sunny day.`;
...
function updateBattCompare() {
  ...
  if (solarBattType === 'lithium') {
    el.innerHTML = `<div class="batt-info batt-info-li">
      <div class="batt-info-col"><h4>Lithium Advantages</h4>
        ...
        <div class="batt-info-item">Charges faster from panels</div>
      </div>
    ...
    el.innerHTML = `<div class="batt-info batt-info-tu">
      <div class="batt-info-col"><h4>Tubular Advantages</h4>
      ...
      <div class="batt-info-col"><h4>Tubular Disadvantages</h4>
        ...
        <div class="batt-info-item neg">Must be in a ventilated space</div>
      </div>
    </div>`;
  }
}
...
function shareCalc(platform) {
  const siteUrl = 'https://chibaikpower.vercel.app';
...
function buildMsg(name) {
  ...
  return `Hi! ${who}. I used your Load Calculator and my total load is ${waLoad.toLocaleString()}W. I am interested in the ${waItemText}. Is it available and how much does it cost?`;
}
...
function sendToChidi() {
  const question = document.getElementById('askQuestion')?.value.trim();
  if (!question) { alert('Please write your question first.'); return; }
...
async function sendChat() {
  ...
  try {
    ...
    if (typing) typing.classList.remove('show');
    appendMsg('bot', 'No internet connection or something went wrong. Please try again.');
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Fuel calc keyboard shortcut
  const fcEl = document.getElementById('fc-p');
  if (fcEl) fcEl.addEventListener('keydown', e => { if (e.key === 'Enter') calcFuel(); });
  const fcLEl = document.getElementById('fc-l');
  if (fcLEl) fcLEl.addEventListener('keydown', e => { if (e.key === 'Enter') calcFuel(); });

  // WA modal keyboard shortcut
